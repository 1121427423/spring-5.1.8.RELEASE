...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
	...
	public int loadBeanDefinitions(Resource resource) throws BeanDefinitionStoreException {
		/**
		 * loadBeanDefinitions: step8
		 * 将resource转成EncodedResource
		 * EncodedResource可进行编码转换
		 */
	...
	public int loadBeanDefinitions(EncodedResource encodedResource) throws BeanDefinitionStoreException {
		/**
		 * 加载xml文件的核心方法，这里是入口，doLoadBeanDefinitions是真正的实现
		 */
		...
		//与ThreadLocal绑定
		// resourcesCurrentlyBeingLoaded 是一个可命名的ThreadLocal实例 name="XML bean definition resources currently being loaded"
		...
		//将encodedResource添加进入currentResources
		...
		try {
			...
			try {
				...
				/**
				 * loadBeanDefinitions: step9
				 * loadBeanDefinitions -> doLoadBeanDefinitions
				 * loadBeanDefinitions真正的实现
				 */
	...
	protected int doLoadBeanDefinitions(InputSource inputSource, Resource resource)
		...
		try {
			//加载xml文档(dom),同时根据是否有DocType决定验证方式(dtd,xsd)，还可以指定实体解析器EntityResorver
			Document doc = doLoadDocument(inputSource, resource);

			/**
			 * loadBeanDefinitions: step10  -> registerBeanDefinitions
			 * 由加载BeanDefinitions阶段过渡到注册BeanDefinitions阶段
			 */
	 ...
	 */
	protected int getValidationModeForResource(Resource resource) {//判断有没有doctype
	 ...
	 * 将加载的Document文档内容进行注册
	 */
	public int registerBeanDefinitions(Document doc, Resource resource) throws BeanDefinitionStoreException {
		//获得文档读取读取器
		BeanDefinitionDocumentReader documentReader = createBeanDefinitionDocumentReader();
		int countBefore = getRegistry().getBeanDefinitionCount();//spring自身注册的实体

		/**
		 * registerBeanDefinitions: step1
		 */
		//进行注册
		documentReader.registerBeanDefinitions(doc, createReaderContext(resource));
		//注册的BeanDefinition实体数量
